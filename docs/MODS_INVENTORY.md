...
- **Last reconciled:** 2026-03-31 — **Active** rows vs `mods/*.pw.toml` (26) + `shaderpacks/*.pw.toml` (1) + `resourcepacks/*.pw.toml` (2), `pack.toml` `0.0.9-alpha`; **Listed** 1 (ParCool!); **Discarded** 1 (**First Person Model**). Default mod configs under `config/` are shipped for client/server sync (see `docs/DEVELOPMENT.md`). Branches **`main`** and **`dev`** share the same mod set unless a future change targets **`dev`** only.
