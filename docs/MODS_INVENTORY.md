...
| Natural Temperature | [modrinth.com/mod/natural-temperature](https://modrinth.com/mod/natural-temperature) | World Gen, Climate | both | high | Fabric API | — | Latitude | Climate bands for a more Earth-like overworld; shipped as `both` here so the singleplayer client carries the same worldgen stack. The pack now ships an active `mode 6` preset plus a `mode 5` comparison preset under `config/natural-temperature-presets/`. |
