...
| --- | --- | --- | --- | --- | --- |
| `0.0.11-alpha` (see `pack.toml`) | `1.21.1` | `0.18.4` | 88 (85 + 1 + 2) | 6 | 2 |
...
| Death Backup | [modrinth.com/mod/death-backup](https://modrinth.com/mod/death-backup) | Gameplay, Utility | both | medium | Collective | — | — | Backup gear on death; shipped as `both` so singleplayer mirrors multiplayer. |
...
| Dismount Entity | [modrinth.com/mod/dismount-entity](https://modrinth.com/mod/dismount-entity) | Gameplay, Utility | both | low | Collective | — | — | Dismount riders; shipped as `both` so player-facing behavior matches multiplayer. |
| Distant Horizons | [modrinth.com/mod/distanthorizons](https://modrinth.com/mod/distanthorizons) | Optimization, Rendering, Multiplayer | both | high | — | — | — | Multiplayer LOD layer; install on both sides, keep `Distant Generation` enabled on server and client, and pregenerate the starter region before public access. |
...
| Fabric/Quilt Chunk Pregenerator | [modrinth.com/mod/fabricquilt-chunk-pregenerator](https://modrinth.com/mod/fabricquilt-chunk-pregenerator) | Management, World Gen, Server Ops | server | medium | Fabric API | — | — | Server-side pregeneration tool; use it for the 128-chunk starter region and keep it out of client installs. |
...
| Leaves Be Gone | [modrinth.com/mod/leaves-be-gone](https://modrinth.com/mod/leaves-be-gone) | Performance, World | both | medium | Fabric API, Puzzles Lib, Forge Config API Port | — | — | Faster leaf decay; shipped as `both` to keep world feel aligned across singleplayer and multiplayer. |
...
| Natural Temperature | [modrinth.com/mod/natural-temperature](https://modrinth.com/mod/natural-temperature) | World Gen, Climate | both | high | Fabric API | — | Latitude | Climate bands for a more Earth-like overworld; shipped as `both` here so the singleplayer client carries the same worldgen stack. The pack now ships an active `mode 5` preset plus a `mode 6` comparison preset under `config/natural-temperature-presets/`. |
...
| No Feather Trample | [modrinth.com/mod/no-feather-trample](https://modrinth.com/mod/no-feather-trample) | Gameplay, Farming | both | low | Collective | — | — | Crop trample tweak; shipped as `both` so crop protection behaves the same in singleplayer. |
...
| Ready Player Fun | [modrinth.com/mod/ready-player-fun](https://modrinth.com/mod/ready-player-fun) | Gameplay, Server | server | low | Fabric API | — | — | Server-side player fun features; stays server-only because it is infrastructure rather than a core player-facing rule. |
...
| RightClickHarvest | [modrinth.com/mod/rightclickharvest](https://modrinth.com/mod/rightclickharvest) | Gameplay, Farming | both | medium | Fabric API, Architectury API, JamLib | — | — | Right-click harvest; shipped as `both` to preserve farming behavior in singleplayer. |
...
| Set World Spawn Point | [modrinth.com/mod/set-world-spawn-point](https://modrinth.com/mod/set-world-spawn-point) | Utility, Server | both | low | Collective | — | — | Exact custom spawn point and spawn tweaks; pin new worlds to `0,0` with `forceExactSpawn`. Shipped as `both` so singleplayer uses the same spawn rule. |
...
| WITS (What Is This Structure?) | [modrinth.com/mod/wits](https://modrinth.com/mod/wits) | Utility, World | both | medium | Fabric API | — | — | Structure identification; shipped as `both` so the command exists in singleplayer too. |
...
- **Last reconciled:** 2026-04-02 — **Active** rows vs `mods/*.pw.toml` (85) + `shaderpacks/*.pw.toml` (1) + `resourcepacks/*.pw.toml` (2), `pack.toml` `0.0.11-alpha`; **Listed** 6 (**Biomes O' Plenty**, **Continents**, **Latitude**, **ParCool!**, **Regions Unexplored**, **Terralith**); **Discarded** 2 (**e4mc**, **First Person Model**). **EasyAuth** and `SimpleAFK` remain **server-only** for dedicated-server administration. Gameplay-affecting server mods that matter in singleplayer are shipped as `both` so integrated-server play matches multiplayer. **Default Options** now covers the vanilla keybind/video baseline, and **Paxi** auto-loads the benchmark datapack from the base `datapacks/` directory plus the ordered global resource packs in `config/paxi/*`. **Pick Up Notifier** replaced the incompatible `ItemPickupNotifier` Modrinth build that declared `minecraft 1.21.10`; this line is aligned with `1.21.1`. **Natural Temperature + Lithostitched + Tectonic + WWOO** remains the recommended overworld baseline, while **Distant Horizons** now anchors the multiplayer LOD path and **Fabric/Quilt Chunk Pregenerator** handles the starter-region pregen on the dedicated server before public access. `World Preview` is still the client-side entry point for comparing seeds and terrain before generation. The pack also ships `config/natural-temperature-presets/mode5.json` and `config/natural-temperature-presets/mode6.json` so you can swap between the two presets locally without rewriting the manifest. **e4mc** was removed from the shipped pack because it conflicted with **Krypton** during integrated-server login and broke standard singleplayer/LAN startup. **Manual smoke (host):** create a fresh singleplayer world; check `Options`/`Controls` for default baseline behavior; verify `benchmark:*` is available without manual world injection; open **JEI** while **InvMove** walking; try **Combat Roll** with **Better Combat**; review **Client Tweaks** vs **Smooth F5**; for worldgen, open `Preview` and compare `Lithostitched + Tectonic`, `Natural Temperature + Lithostitched + Tectonic`, and the fantasy branches listed above; for multiplayer baselines, confirm DH F3 support and pregenerated starter chunks before letting other players join.
