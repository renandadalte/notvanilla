...
| Worldgen, preview e baseline | Camada de clima, macro-terreno, suporte de worldgen e pré-visualização de seed. | Natural Temperature, Lithostitched, Tectonic, William Wythers' Overhauled Overworld, World Preview | — | Abrir o `Preview` no menu Singleplayer, confirmar que `Natural Temperature` e `Lithostitched` carregam junto de `Tectonic`, e comparar biome map/heightmap/structures em um mundo novo com `16`, `32` e `64` chunks. Veja também [WORLDGEN_BASELINE.md](./WORLDGEN_BASELINE.md) para os perfis exatos. |
