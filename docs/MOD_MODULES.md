...
| Worldgen, preview e baseline | Camada de clima, macro-terreno, suporte de worldgen e pré-visualização de seed. | Latitude, Lithostitched, Tectonic, William Wythers' Overhauled Overworld, World Preview | — | Abrir o `Preview` no menu Singleplayer, confirmar que `Lithostitched` carrega junto de `Tectonic`, e comparar biome map/heightmap/structures em um mundo novo com `16`, `32` e `64` chunks. Veja também [WORLDGEN_BASELINE.md](./WORLDGEN_BASELINE.md) para os perfis exatos. |
