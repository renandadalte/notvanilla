...
| Worldgen, preview e baseline | Camada de clima, macro-terreno, suporte de worldgen e pré-visualização de seed. | Natural Temperature, Lithostitched, Tectonic, William Wythers' Overhauled Overworld, World Preview | `config/natural-temperature.json`, `config/natural-temperature-presets/*` | Abrir o `Preview` no menu Singleplayer, confirmar que `Natural Temperature` e `Lithostitched` carregam junto de `Tectonic`, e comparar biome map/heightmap/structures em um mundo novo com `16`, `32` e `64` chunks. Veja também [WORLDGEN_BASELINE.md](./WORLDGEN_BASELINE.md) para os perfis exatos e para a troca entre os presets `mode 5` e `mode 6`. |
| Long-distance rendering e pregeneration | Camada de LOD e preparação do mapa inicial para multiplayer e testes de alcance. | Distant Horizons, Fabric/Quilt Chunk Pregenerator | `world/data/DistantHorizons.sqlite` no servidor, comandos `/dh pregen` e `/pregen start 128` | Confirmar o F3 do DH, pregenerar a área inicial e verificar que o login já carrega LODs visíveis sem explorar a pé. |
| Regras do servidor e multiplayer | Serviços e regras que fazem sentido no mundo compartilhado. | Death Backup, Dismount Entity, EasyAuth, Hand Over Your Items, Leaves Be Gone, No Feather Trample, Ready Player Fun, RightClickHarvest, Set World Spawn Point, SimpleAFK, WITS | `config/EasyAuth/*` no host e configs locais do servidor quando aplicável | Validar login, whitelist, AFK, spawn, agricultura, backup, troca de itens e identificação de estruturas. Os mods que mudam a experiência do jogador no mundo são distribuídos como `both`; `EasyAuth` e `SimpleAFK` continuam `server` por serem infraestrutura. |
...
- Mudou worldgen ou biomas? Comece em `Worldgen, preview e baseline`, leia [WORLDGEN_BASELINE.md](./WORLDGEN_BASELINE.md) e depois valide `Performance e renderização`.
